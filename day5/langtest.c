/*
    DAY 5
    https://www.youtube.com/watch?v=jaLwNQPU6JI
    
    Fixed critical keyword parsing bug
    Added block statements
    Added for loop statements
    Added comparisons/equality binary expressions
    
    Generating very brittle looping instructions as assembly text for the VM
    Backpatching the string with the instruction index to jump to
    Lots to improve, ideally we would simply generate the bytecode directly
    But we are generating text/string for easier debugging 
    (and the VM needs to parse strings to generate bytecode)
