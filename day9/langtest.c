/*
    ...
    added tok_SP and tok_FP for explicit frame pointer and stack pointer

    Fixed a bug in the keyword generator, takes a section of a larger word and generates a keyword token
    example: generates tok_SP for 'spoon'

    changed memory mov commands to explicit LOAD and STORE
    LOAD = move some memory onto a register
    STORE = move some data(reg/imm) into memory


    added math/comparisons/jumps/inc/dec/push/pop bytecode emissions, and to the vm runtime
    validated fibonacci works
...
/*
    ...
    add TONS of printf/debugging logs to the vm runtime
        - print jump count / where we're jumping to in an instruction
        - print math operations
        - print comparisons
        - print what the flag is
        - basically print all the internal state being mutated at any instruction

    sandbox out of memory accessing
...
int keywordMatch(char* str, int start, int len, const char* compare, int size){
    ...
    //TODO: guard against out of bound accessing here
    if(isAlpha(*(str + start + size)) || isNum(*(str + start + size)))return 0;
    return 1;
}
        ...
        switch(vm.bytecode[vm.ip]){
            ...
            case op_push_reg:{
                ...
                if((vm.registers[STACK_POINTER] - 4) >= MAX_VM_MEM || (vm.registers[STACK_POINTER] - 4) < 0){
                    Assert(0 && "STACK POINTER OUT OF BOUNDS");
                }
            ...
            case op_store_addreg_imm:{
                ...
                //TODO: sandbox this, check if addr is out of bounds
