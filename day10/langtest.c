...
/*
    ...
    figure out how to handle immediate values larger than 1 byte
    do a we do a constant lookup table? do we allow for flexible/more operators for variable sized immediates?

    negative numbers - unary symbol to invert constant/immediate number tokens

    directives .u32 x 100 to define data before the vm instructions run
    labels x: to tell the code where to jump to/return from

    ADD     r0      1290478901725897192857
    ADD     r0      -1290478901725897192857
    1       1       4                           = 6 bytecodes, currently we can only handle 4 bytecodes

    HLT/RET VM operators HLT = halts/exits the VM, RET returns from an instruction
    SYSCALL operator to setup external/native function calls, need to define ABI (application binary interface)
...
/*
    ASSUMPTIONS:
    all immediate values are 1 byte
    assume EVERY NUMERICAL VALUE IS 1 BYTE

    globals are actually stored on the stack, will break when we add recursion
    need to store globals in explicit global memory/at the start of memory

    no comparisons between immediates, only between register and immediate

    once everything works, we can expand to more bytes
    need to adjust the stack push/pop immediately to store more than one byte at a time
...
#define DEBUG 1
    ...
    #define VMPDEBUG(...) printf(__VA_ARGS__);
#else
    #define PDEBUG(...)
    #define VMPDEBUG(...)
#endif
...
typedef struct VM{
    ...
    //constraints
    //stop executing if we hit the budget
    int budget;

    //stop executing if we hit the jumpcount
    int jumpcount;
...
void resetVM(VM* vm){
    ...
    vm->registers[STACK_POINTER]= (MAX_VM_MEM) - 4;
    vm->registers[FRAME_POINTER] = vm->registers[STACK_POINTER];
}
...
VM vm = {0};
char vmInput[MAX_STRING_SIZE];
token vmTokens[MAX_STRING_SIZE];
char vmVarStorage[MAX_STRING_SIZE];
int vmVarStorageOffset = 0;
int vmParserLine = 1;
char* vmInputLines[MAX_STRING_SIZE];
token vmTokens[MAX_STRING_SIZE];
int vmTokenCount = 0;
char tempVMLine[MAX_STRING_SIZE];
int tempVMLineCount = 0;


char input[MAX_STRING_SIZE];
token tokens[MAX_STRING_SIZE];
expr exprs[MAX_STRING_SIZE];
stmt stmts[MAX_STRING_SIZE]; //arena for statement allocation
block_link blockLinks[MAX_STRING_SIZE];
int blockLinkCount = 0;

stmt* globalStmts[MAX_STRING_SIZE]; //sequential list of statements within blocks
int globalStmtCount = 0;

char varStorage[MAX_STRING_SIZE];
hashEntry hashTable[HASH_TABLE_SIZE];
localsHashEntry localsHashTable[16][HASH_TABLE_SIZE];
int maxScope = 16;
int currentScope = 0;

object objects[MAX_STRING_SIZE];
int objectCount = 0;

char astStr[MAX_STRING_SIZE];
int astStrCount = 0;
...
int parserLine = 1;
char* inputLines[MAX_STRING_SIZE];

char tempErrorLine[MAX_STRING_SIZE];
int tempErrorLineCount = 0;


char byteStr[MAX_STRING_SIZE];
int byteStrCount = 0;
...
const char* tokStr(token_type t){
    switch(t){
        ...
        case tok_LOAD:{return "tok_LOAD";}break;
        case tok_STORE:{return "tok_STORE";}break;
        case tok_FP:{return "tok_FP";}break;
        case tok_SP:{return "tok_SP";}break;
        default:{}return "";
    }
    return "TEST";
}
...
int keywordMatch(char* str, int start, int len, const char* compare, int size){
    int loweroffset = 'a' - 'A';
    for(int i = 0; i < size; ++i){

        if(i + start >= len)
        {
            //TODO: just return a 0 here, no match
            Assert(0);//handle this later, what happens if we hit the end and try to look for a keyword?
            break;
        }
    ...
    if(start + size >= len)return 1;//guard against out of bound accessing here
    if(isIdent(*(str + start + size)))return 0;
    return 1;
}

token_type isKeyword(char* str, int i, int len){
    char c = str[i++];
    ...
    switch(c){//trie
        case 'i':{//int
            if(str[i] == 'f'){
                ...
                if(keywordMatch(str, i, len, "nt", 2)){
                    type = tok_int;
                }
            }
        }break;
        case 'b':{//bool
            if(keywordMatch(str, i, len, "ool", 3)){
                type = tok_bool;
            }
        }break;
        case 'c':{//char
            if(keywordMatch(str, i, len, "har", 3)){
                type = tok_char;
            }
        }break;
        case 't':{//true
            if(keywordMatch(str, i, len, "rue", 3)){
                type = tok_true;
            }
        }break;
        case 'f':{
            if(str[i] == 'o'){
                ...
                if(keywordMatch(str, i, len, "alse", 4)){
                    type = tok_false;
                }
            }
        ...
        case 'e':{//else / elseif
            if(keywordMatch(str, i, len, "lse if", 6)){
                type = tok_elseif;
            }else{
                if(keywordMatch(str, i, len, "lse", 3)){
                    type = tok_else;
                }
            }
        }break;
        case 'w':{//while
            if(keywordMatch(str, i, len, "hile", 4)){
                type = tok_while;
            }
        }break;
...
token_type isVMKeyword(char* str, int i, int len){
    char c = str[i++];
    token_type type = tok_none;
    int loweroffset = 'a' - 'A';
    if(c >= 'a'){//lowercase
    ...
    switch(c){//trie
        case 'a':{if(keywordMatch(str, i, len, "dd", 2))type = tok_ADD;}break;
        case 'd':{
                 if(keywordMatch(str, i, len, "iv", 2))type = tok_DIV;
            else if(keywordMatch(str, i, len, "ec", 2))type = tok_DEC;
        }break;
        case 'e':{if(keywordMatch(str, i, len, "q", 1))type = tok_EQ;}break;
        case 'f':{if(keywordMatch(str, i, len, "p", 1))type = tok_FP;}break;
        case 'g':{
            if      (keywordMatch(str, i, len, "te", 2))type = tok_GTE;
            else if (keywordMatch(str, i, len, "t", 1))type = tok_GT;
        }break;
        case 'i':{if(keywordMatch(str, i, len, "nc", 2))type = tok_INC;}break;
        case 'j':{
            if      (keywordMatch(str, i, len, "mp", 2))type = tok_JMP;
            else if (keywordMatch(str, i, len, "eq", 2))type = tok_JEQ;
            else if (keywordMatch(str, i, len, "ne", 2))type = tok_JNE;
        }break;
        case 'l':{
            if      (keywordMatch(str, i, len, "te", 2))type = tok_LTE;
            else if (keywordMatch(str, i, len, "t", 1))type = tok_LT;
            else if (keywordMatch(str, i, len, "oad", 3))type = tok_LOAD;
        }break;
        case 'm':{//ov
            if     (keywordMatch(str, i, len, "ov32", 4))type = tok_MOV32;
            else if(keywordMatch(str, i, len, "ov16", 4))type = tok_MOV16;
            else if(keywordMatch(str, i, len, "ov8", 3))type = tok_MOV8;
            else if(keywordMatch(str, i, len, "ov", 2))type = tok_MOV;
            else if(keywordMatch(str, i, len, "ul", 2))type = tok_MUL;
        }break;
        case 'n':{if(keywordMatch(str, i, len, "e", 1))type = tok_NE;}break;
        case 'p':{
                 if(keywordMatch(str, i, len, "op", 2))type = tok_POP;
            else if(keywordMatch(str, i, len, "ush", 3))type = tok_PUSH;
        }break;
        case 's':{
                 if(keywordMatch(str, i, len, "p", 1))type = tok_SP;
            else if(keywordMatch(str, i, len, "ub", 2))type = tok_SUB;
            else if(keywordMatch(str, i, len, "tore", 4))type = tok_STORE;
        }break;
...
//TODO: add a check for '_'
//something like isIdent that checks isAlpha, isNum, and '_'
...
void parser(char* input, int len){
    ...
    while(input[i] != '\0'){
        ...
        if(isNum(c)){
            ...
            token_type type = isKeyword(input, i, len);
            if(type){
            ...
            switch(c){
                ...
                case '/':{token t = {0}; t.type = tok_slash;         addToken(t); i++;}break;
                case '*':{token t = {0}; t.type = tok_star;          addToken(t); i++;}break;
                case ';':{token t = {0}; t.type = tok_semicolon;     addToken(t); i++;}break;
                case '(':{token t = {0}; t.type = tok_lparen;        addToken(t); i++;}break;
                case ')':{token t = {0}; t.type = tok_rparen;        addToken(t); i++;}break;
                case '{':{token t = {0}; t.type = tok_lcurly;        addToken(t); i++;}break;
                case '}':{token t = {0}; t.type = tok_rcurly;        addToken(t); i++;}break;
                case '[':{token t = {0}; t.type = tok_lbrack;        addToken(t); i++;}break;
                case ']':{token t = {0}; t.type = tok_rbrack;        addToken(t); i++;}break;
                case '!':{
                    token t = {0}; 
                    if(input[i+1] == '='){
                        t.type = tok_nequal;
...
void printTokens(token* tokens, int tokenCount, char* varStorage){
    for(int i = 0; i < tokenCount; ++i){
        ...
        switch(t.type){
            ...
            case tok_LOAD       :{printf("tok_LOAD       %3d\n", i);}break;
            case tok_STORE      :{printf("tok_STORE      %3d\n", i);}break;
            case tok_FP         :{printf("tok_FP         %3d\n", i);}break;
            case tok_SP         :{printf("tok_SP         %3d\n", i);}break;
            default:{}break;
        }
    }
}
...
void vmParser(char* input, int len){
    int i = 0;
    char c = input[i];
    while(input[i] != '\0'){
        // printf("%c | isAlpha : %d, isNum : %d, isWhitespace %d\n", c, isAlpha(c), isNum(c), isWhitespace(c));
        ...
        if(isNum(c)){//TODO add floats, only handles integers
            ...
            token_type type = isVMKeyword(input, i, len);
            if(type){
                printf("ASM KEYWORD FOUND | %s\n", tokStr(type));
                token t = {0};
                switch(type){
                    case tok_MOV        :{i += 3; t.type = tok_MOV        ;}break;
                    case tok_LOAD       :{i += 4; t.type = tok_LOAD       ;}break;
                    case tok_STORE      :{i += 5; t.type = tok_STORE      ;}break;
                    case tok_FP         :{i += 2; t.type = tok_FP         ;}break;
                    case tok_SP         :{i += 2; t.type = tok_SP         ;}break;
                    ...
                    case tok_PUSH       :{i += 4; t.type = tok_PUSH       ;}break;
                    case tok_POP        :{i += 3; t.type = tok_POP        ;}break;
            ...
            switch(c){
                ...
                case '/':{token t = {0}; t.type = tok_slash;         vmAddToken(t); i++;}break;
                case '*':{token t = {0}; t.type = tok_star;          vmAddToken(t); i++;}break;
                case ';':{
                    while(input[i] != '\n' && input[i] != 0){//loop until we hit the newline
                        i++;
                    }
                }break;
                case '(':{token t = {0}; t.type = tok_lparen;        vmAddToken(t); i++;}break;
                case ')':{token t = {0}; t.type = tok_rparen;        vmAddToken(t); i++;}break;
                case '{':{token t = {0}; t.type = tok_lcurly;        vmAddToken(t); i++;}break;
                case '}':{token t = {0}; t.type = tok_rcurly;        vmAddToken(t); i++;}break;
                case '[':{token t = {0}; t.type = tok_lbrack;        vmAddToken(t); i++;}break;
                case ']':{token t = {0}; t.type = tok_rbrack;        vmAddToken(t); i++;}break;
                case '!':{
                    token t = {0}; 
                    if(input[i+1] == '='){
                        t.type = tok_nequal;
...
typedef enum {
    ...
    //[#0] $0 addr_reg
    //$0 [] reg_addr
    op_store_addreg_imm, //[$0] #0 
    op_store_addimm_imm, //[#0] #0
    op_store_addreg_reg, //[$0] $0 
    op_store_addimm_reg, //[#0] $0
    op_load_reg_addreg, // $0 [$0]
    op_load_reg_addimm, // $0 [#0]
}op_type; //operators
...
const char* opStr(int op){
    switch(op){
        ...
        case op_sub_reg_imm     :{return "op_sub_reg_imm";}break;
        case op_mul_reg_reg     :{return "op_mul_reg_reg";}break;
        case op_mul_reg_imm     :{return "op_mul_reg_imm";}break;
        case op_div_reg_reg     :{return "op_div_reg_reg";}break;
        case op_div_reg_imm     :{return "op_div_reg_imm";}break;
        case op_push_reg        :{return "op_push_reg";}break;
        case op_push_imm        :{return "op_push_imm";}break;
        case op_pop_reg         :{return "op_pop_reg";}break;
        case op_inc_reg         :{return "op_inc_reg";}break;
        case op_dec_reg         :{return "op_dec_reg";}break;
        case op_EQ_reg_reg      :{return "op_EQ_reg_reg";}break;
        case op_EQ_reg_imm      :{return "op_EQ_reg_imm";}break;
        case op_NE_reg_reg      :{return "op_NE_reg_reg";}break;
        case op_NE_reg_imm      :{return "op_NE_reg_imm";}break;
        case op_LT_reg_reg      :{return "op_LT_reg_reg";}break;
        case op_LT_reg_imm      :{return "op_LT_reg_imm";}break;
        case op_GT_reg_reg      :{return "op_GT_reg_reg";}break;
        case op_GT_reg_imm      :{return "op_GT_reg_imm";}break;
        case op_LTE_reg_reg     :{return "op_LTE_reg_reg";}break;
        case op_LTE_reg_imm     :{return "op_LTE_reg_imm";}break;
        case op_GTE_reg_reg     :{return "op_GTE_reg_reg";}break;
        case op_GTE_reg_imm     :{return "op_GTE_reg_imm";}break;
        case op_JMP_reg         :{return "op_JMP_reg";}break;
        case op_JMP_imm         :{return "op_JMP_imm";}break;
        case op_JEQ_reg         :{return "op_JEQ_reg";}break;
        case op_JEQ_imm         :{return "op_JEQ_imm";}break;
        case op_JNE_reg         :{return "op_JNE_reg";}break;
        case op_JNE_imm         :{return "op_JNE_imm";}break;
        case op_store_addreg_imm:{return "op_store_addreg_imm";}break;
        case op_store_addimm_imm:{return "op_store_addimm_imm";}break;
        case op_store_addreg_reg:{return "op_store_addreg_reg";}break;
        case op_store_addimm_reg:{return "op_store_addimm_reg";}break;
        case op_load_reg_addreg :{return "op_load_reg_addreg";}break;
        case op_load_reg_addimm :{return "op_load_reg_addimm";}break;
...
void runVM(){
    vm.ip = 0;
    if(vm.jumpcount > 32){
        printf("VM JUMP COUNT OF 32 EXCEEDED! : %d\n", vm.jumpcount);
        Assert(0);
    }
    ...
    while(vm.ip < vm.bytecodeCount){
        ...
        printf("CURRENT INSTRUCTION %s%s%s\n", BRED, opStr(vm.bytecode[curip]), RESET);
        printVMLine((curip / 4) + 1);
        switch(vm.bytecode[curip]){
            case op_mov_reg_reg:{
                u8 reg1 = vm.bytecode[curip + 1];
                u8 reg2 = vm.bytecode[curip + 2];
                VMPDEBUG("r%d = r%d | %d = %d == %d\n", reg1, reg2, vm.registers[reg1], vm.registers[reg2], vm.registers[reg2]);
            ...
            case op_mov_reg_imm:{
                ...
                imm += (vm.bytecode[curip + 2] << 0);
                imm += (vm.bytecode[curip + 3] << 8);
                
                VMPDEBUG("r%d = %d | %d = %d == %d\n", reg1, imm, vm.registers[reg1], imm, imm);
            ...
            case op_add_reg_reg:{
                u8 reg1 = vm.bytecode[curip + 1];
                u8 reg2 = vm.bytecode[curip + 2];
                VMPDEBUG("r%d + r%d | %d = %d == %d\n", reg1, reg2, vm.registers[reg1], vm.registers[reg2], vm.registers[reg1] + vm.registers[reg2]);
            ...
            case op_add_reg_imm:{
                u8 reg1 = vm.bytecode[curip + 1];
                u8 imm = vm.bytecode[curip + 2];
                VMPDEBUG("r%d + r%d | %d = %d == %d\n", reg1, imm, vm.registers[reg1], imm, vm.registers[reg1] + imm);
                vm.registers[reg1] = vm.registers[reg1] + imm;
            }break;
            case op_sub_reg_imm:{
                u8 reg1 = vm.bytecode[curip + 1];
                u8 imm = vm.bytecode[curip + 2];
            ...
            case op_push_reg:{
                u8 reg1   = vm.bytecode[curip + 1];
                vm.mem[vm.registers[STACK_POINTER]] = vm.registers[reg1];
                if((vm.registers[STACK_POINTER] - 4) >= MAX_VM_MEM || (vm.registers[STACK_POINTER] - 4) < 0){
                    printf("STACK POINTER AT %d OUT OF BOUNDS! BREAKING OUT OF VM RUNTIME!\n", vm.registers[STACK_POINTER] - 4);
                    break;
                }
                vm.registers[STACK_POINTER] -= 4;
                VMPDEBUG("Pushed r%d = %d onto stack location %d, advanced SP to %d\n", reg1, vm.registers[reg1], vm.registers[STACK_POINTER] + 4, vm.registers[STACK_POINTER]);
            }break;
            case op_push_imm:{
                u8 imm   = vm.bytecode[curip + 1];
                vm.mem[vm.registers[STACK_POINTER]] = imm;
                if((vm.registers[STACK_POINTER] - 4) >= MAX_VM_MEM || (vm.registers[STACK_POINTER] - 4) < 0){
                    printf("STACK POINTER AT %d OUT OF BOUNDS! BREAKING OUT OF VM RUNTIME!\n", vm.registers[STACK_POINTER] - 4);
                    break;
                }
                vm.registers[STACK_POINTER] -= 4;
                VMPDEBUG("Pushed %d onto stack location %d, advanced SP to %d\n", imm, vm.registers[STACK_POINTER] + 4, vm.registers[STACK_POINTER]);
            }break;
            case op_pop_reg:{
                u8 reg1   = vm.bytecode[curip + 1];
                if((vm.registers[STACK_POINTER] + 4) >= MAX_VM_MEM || (vm.registers[STACK_POINTER] + 4) < 0){
                ...
                vm.registers[STACK_POINTER] += 4;
                vm.registers[reg1] = vm.mem[vm.registers[STACK_POINTER]];
                VMPDEBUG("Popped %d onto r%d | stack location %d, advanced SP to %d\n", reg1, vm.mem[vm.registers[STACK_POINTER]], vm.registers[STACK_POINTER] + 4, vm.registers[STACK_POINTER]);
            }break;

            //STORE
            case op_store_addreg_imm:{
                u8 reg1   = vm.bytecode[curip + 1];
                u8 offset = vm.bytecode[curip + 2];
                u8 imm    = vm.bytecode[curip + 3];
                u32 addr = vm.registers[reg1] + offset;
                if(addr >= MAX_VM_MEM || addr < 0){
                    printf("ERROR ADDRESSING OUTSIDE OF VM MEMORY! addr: %d\n", addr);
                    break;
                    //increment some vm error here
                }
                vm.mem[addr] = imm;
                VMPDEBUG("Storing %d to addr: %d\n", imm, addr);
                //TODO: expand to setting 4 bytes at a time for default MOV
            }break;
            case op_store_addimm_imm:{
                u8 imm1   = vm.bytecode[curip + 1];
                u8 offset = vm.bytecode[curip + 2];
                u8 imm2    = vm.bytecode[curip + 3];
            ...
            case op_store_addreg_reg:{
                u8 reg1   = vm.bytecode[curip + 1];
                u8 offset = vm.bytecode[curip + 2];
                u8 reg2    = vm.bytecode[curip + 3];
                u32 addr = vm.registers[reg1] + offset;
                if(addr >= MAX_VM_MEM || addr < 0){
                    printf("ERROR ADDRESSING OUTSIDE OF VM MEMORY! addr: %d\n", addr);
                    break;
                }
                vm.mem[addr] = vm.registers[reg2];
                VMPDEBUG("Storing r%d = %d to addr: %d\n",reg2, vm.registers[reg2], addr);
            }break;
            case op_store_addimm_reg:{
                u8 imm   = vm.bytecode[curip + 1];
                u8 offset = vm.bytecode[curip + 2];
                u8 reg    = vm.bytecode[curip + 3];
                u32 addr = imm + offset;
                if(addr >= MAX_VM_MEM || addr < 0){
                    printf("ERROR ADDRESSING OUTSIDE OF VM MEMORY! addr: %d\n", addr);
                    break;
                }
                vm.mem[addr] = vm.registers[reg];
                VMPDEBUG("Storing r%d = %d to addr: %d\n",reg, vm.registers[reg], addr);
            }break;

            //LOAD
            case op_load_reg_addreg:{//MOV $0 [$0 + offset]
                u8 reg1     = vm.bytecode[curip + 1];
                u8 reg2     = vm.bytecode[curip + 2];
                u8 offset   = vm.bytecode[curip + 3];
                u32 addr = vm.registers[reg2] + offset;
                if(addr >= MAX_VM_MEM || addr < 0){
                    printf("ERROR ACCESSING OUTSIDE OF VM MEMORY! addr: %d\n", addr);
                    break;
                }
                vm.registers[reg1] = vm.mem[addr];
                VMPDEBUG("Loading addr %d = %d to r%d\n", addr, vm.mem[addr], reg1);
            }break;
            case op_load_reg_addimm:{
                u8 reg1     = vm.bytecode[curip + 1];
                u8 imm     = vm.bytecode[curip + 2];
                u8 offset   = vm.bytecode[curip + 3];
                u32 addr = imm + offset;
            ...
            case op_inc_reg:{
                u8 reg1 = vm.bytecode[curip + 1];
                vm.registers[reg1] += 1;
                VMPDEBUG("INC r%d = %d == %d\n", reg1, vm.registers[reg1]-1, vm.registers[reg1]);

            }break;
            case op_dec_reg:{
                u8 reg1 = vm.bytecode[curip + 1];
                vm.registers[reg1] -= 1;
                VMPDEBUG("DEC r%d = %d == %d\n", reg1, vm.registers[reg1]+1, vm.registers[reg1]);
            ...
            case op_EQ_reg_reg :{
                ...
                VMPDEBUG("r%d == r%d | %d == %d flag: %d\n", reg1, reg2, vm.registers[reg1], vm.registers[reg2], vm.flag);
            }break;
            case op_NE_reg_reg :{
                u8 reg1 = vm.bytecode[curip + 1];
                u8 reg2 = vm.bytecode[curip + 2];
            ...
            case op_EQ_reg_imm :{
                u8 reg1 = vm.bytecode[curip + 1];
                u8 imm = vm.bytecode[curip + 2];
                if(vm.registers[reg1] == imm){
                    vm.flag = 1;
                }else{
                    vm.flag = 0;
                }
                VMPDEBUG("r%d == %d | %d == %d flag: %d\n", reg1, imm, vm.registers[reg1], imm, vm.flag);
            }break;
            case op_NE_reg_imm :{
                u8 reg1 = vm.bytecode[curip + 1];
                u8 imm = vm.bytecode[curip + 2];
            ...
            case op_JMP_reg:{
                u8 reg1 = vm.bytecode[curip + 1];
                vm.ip = vm.registers[reg1];
                if(vm.ip < 0 || vm.ip >= vm.bytecodeCount){
                    printf("vm.ip = %d, jumped out of bytecodeCount: %d, exiting VM\n", vm.ip, vm.bytecodeCount);
                    break;
                }
                vm.jumpcount++;
                VMPDEBUG("JMP vm.ip = %d, jumpcount: %d\n", vm.ip, vm.jumpcount);
            }break;
            case op_JMP_imm:{
                u8 imm = vm.bytecode[curip + 1];
                vm.ip = imm;
            ...
            case op_JEQ_reg:{
                ...
                if(vm.flag){
                    ...
                    vm.jumpcount++;
                    VMPDEBUG("JEQ vm.ip = %d, jumpcount: %d\n", vm.ip, vm.jumpcount);
                }
            }break;
            case op_JEQ_imm:{
                u8 imm = vm.bytecode[curip + 1];
            ...
            case op_JNE_reg:{
                ...
                if(!vm.flag){
                    ...
                    vm.jumpcount++;
                    VMPDEBUG("JNE vm.ip = %d, jumpcount: %d\n", vm.ip, vm.jumpcount);
                }
            }break;
            case op_JNE_imm:{
                u8 imm = vm.bytecode[curip + 1];
        ...
        //TODO: move this to the top and create a local to base off in the instructions
        //when we jump, we go to the instruction, but immediately increment
    }
    printf("vm.ip = %d, outside of instruction set, bytecodeCount = %d\n", vm.ip, vm.bytecodeCount);
    printf("VM SCRIPT COMPLETE!\n");
...
operand determineOperand(token* tokens, int* nextToken){
    ...
    switch(t.type){
        ...
        case tok_FP:{
            op.type = operand_reg;
            op.regOrImm = FRAME_POINTER;
            (*nextToken)++;
        }break;
        case tok_SP:{
            op.type = operand_reg;
            op.regOrImm = STACK_POINTER;
            (*nextToken)++;
        }break;
...
void vmEmitBytecode(token* tokens, int tokenCount){
    ...
    while(i < tokenCount){
        ...
        if(top.type != tok_none){
            ...
            //MOV
            if(top.type == tok_MOV){
                if(op1.type == operand_reg && op2.type == operand_reg && op3.type == operand_none){
                    optype = op_mov_reg_reg;
                }else if(op1.type == operand_reg && op2.type == operand_imm && op3.type == operand_none){
            ...
            else if(top.type == tok_SUB){
                if(op1.type == operand_reg && op2.type == operand_reg && op3.type == operand_none){
                    optype = op_sub_reg_reg;
                }else if(op1.type == operand_reg && op2.type == operand_imm && op3.type == operand_none){
                    optype = op_sub_reg_imm;
                }
            }
            else if(top.type == tok_MUL){
                if(op1.type == operand_reg && op2.type == operand_reg && op3.type == operand_none){
                    optype = op_mul_reg_reg;
                }else if(op1.type == operand_reg && op2.type == operand_imm && op3.type == operand_none){
                    optype = op_mul_reg_imm;
                }
            }
            else if(top.type == tok_DIV){
                if(op1.type == operand_reg && op2.type == operand_reg && op3.type == operand_none){
                    optype = op_div_reg_reg;
                }else if(op1.type == operand_reg && op2.type == operand_imm && op3.type == operand_none){
                    optype = op_div_reg_imm;
                }
            }

            //COMPARISONS
            else if(top.type == tok_EQ) {
                if(op1.type == operand_reg && op2.type == operand_reg && op3.type == operand_none){
                    optype = op_EQ_reg_reg;
                }else if(op1.type == operand_reg && op2.type == operand_imm && op3.type == operand_none){
                    optype = op_EQ_reg_imm;
                }
            }
            else if(top.type == tok_NE) {
                if(op1.type == operand_reg && op2.type == operand_reg && op3.type == operand_none){
                    optype = op_NE_reg_reg;
                }else if(op1.type == operand_reg && op2.type == operand_imm && op3.type == operand_none){
                    optype = op_NE_reg_imm;
                }
            }
            else if(top.type == tok_LT) {
                if(op1.type == operand_reg && op2.type == operand_reg && op3.type == operand_none){
                    optype = op_LT_reg_reg;
                }else if(op1.type == operand_reg && op2.type == operand_imm && op3.type == operand_none){
                    optype = op_LT_reg_imm;
                }
            }
            else if(top.type == tok_GT) {
                if(op1.type == operand_reg && op2.type == operand_reg && op3.type == operand_none){
                    optype = op_GT_reg_reg;
                }else if(op1.type == operand_reg && op2.type == operand_imm && op3.type == operand_none){
                    optype = op_GT_reg_imm;
                }
            }
            else if(top.type == tok_LTE){
                if(op1.type == operand_reg && op2.type == operand_reg && op3.type == operand_none){
                    optype = op_LTE_reg_reg;
                }else if(op1.type == operand_reg && op2.type == operand_imm && op3.type == operand_none){
                    optype = op_LTE_reg_imm;
                }
            }
            else if(top.type == tok_GTE){
                if(op1.type == operand_reg && op2.type == operand_reg && op3.type == operand_none){
                    optype = op_GTE_reg_reg;
                }else if(op1.type == operand_reg && op2.type == operand_imm && op3.type == operand_none){
                    optype = op_GTE_reg_imm;
                }
            }

            //JUMPS
            else if(top.type == tok_JMP && op2.type == operand_none && op3.type == operand_none){
                if(op1.type == operand_reg){
                    optype = op_JMP_reg;
                }else if(op1.type == operand_imm){
                    optype = op_JMP_imm;
                }
            }
            else if(top.type == tok_JEQ && op2.type == operand_none && op3.type == operand_none){
                if(op1.type == operand_reg){
                    optype = op_JEQ_reg;
                }else if(op1.type == operand_imm){
                    optype = op_JEQ_imm;
            ...
            else if(top.type == tok_JNE && op2.type == operand_none && op3.type == operand_none){
                if(op1.type == operand_reg){
                    optype = op_JNE_reg;
                }else if(op1.type == operand_imm){
                    optype = op_JNE_imm;
            ...
            //INC / DEC
            else if(top.type == tok_INC){
                if(op1.type == operand_reg && op2.type == operand_none && op3.type == operand_none){
                    optype = op_inc_reg;
                }
            }
            else if(top.type == tok_DEC){            
                if(op1.type == operand_reg && op2.type == operand_none && op3.type == operand_none){
                    optype = op_dec_reg;
                }
            }
            ...
            //STORES
            else if(top.type == tok_STORE){
                      if(op1.type == operand_addr_reg && op2.type == operand_imm && op3.type == operand_none){
                    optype = op_store_addreg_imm;
                }else if(op1.type == operand_addr_imm && op2.type == operand_imm && op3.type == operand_none){
                    optype = op_store_addimm_imm;
                }else if(op1.type == operand_addr_reg && op2.type == operand_reg && op3.type == operand_none){
                    optype = op_store_addreg_reg;
                }else if(op1.type == operand_addr_imm && op2.type == operand_reg && op3.type == operand_none){
                    optype = op_store_addimm_reg;
                }
            }
            //LOADS
            else if(top.type == tok_LOAD){
                if(op1.type == operand_reg && op2.type == operand_addr_reg && op3.type == operand_none){
                    optype = op_load_reg_addreg;
                }else if(op1.type == operand_reg && op2.type == operand_addr_imm && op3.type == operand_none){
                    optype = op_load_reg_addimm;
                }
            }
            //PUSH
            else if(top.type == tok_PUSH){
                if(op1.type == operand_reg && op2.type == operand_none && op3.type == operand_none){
                    optype = op_push_reg;
                }
                 
                else if(op1.type == operand_imm && op2.type == operand_none && op3.type == operand_none){
                    optype = op_push_imm;
                }

            }
            //POP
            else if(top.type == tok_POP){
                if(op1.type == operand_reg && op2.type == operand_none && op3.type == operand_none){
                    optype = op_pop_reg;
                }
...
void parseEmitRun(char* input, int len){
    vmInputLines[1] = input;
    vmParser(input, len);
    printTokens(vmTokens, vmTokenCount, vmVarStorage);
...
void vmTest_MOV_ADD(){
    ...
    char asminput[MAX_STRING_SIZE] = 
    "MOV r0 #1\n"
    "MOV r1 #2\n"
    "ADD r0 r1\n"
    "ADD r1 r0\n";
    
    parseEmitRun(asminput, MAX_STRING_SIZE);
...
void vmTest_FP(){
    ...
    ;
    parseEmitRun(asminput, MAX_STRING_SIZE);
    Assert(vm.registers[FRAME_POINTER] == 2);
}


void vmTest_PUSH_POP(){
    resetGlobalVMState();
    char asminput[MAX_STRING_SIZE] = 
    "MOV r0 #3\n"
    "PUSH r0\n"
    "POP r1\n"
    ;
    int stackStart = vm.registers[STACK_POINTER];
    parseEmitRun(asminput, MAX_STRING_SIZE);
    Assert(vm.registers[1] == 3);
    Assert(vm.registers[STACK_POINTER] == stackStart);
...
int main(){
    ...
    while(1){
        ...
        astStrCount = 0;
        errors = 0;
        memset(varStorage, 0, MAX_STRING_SIZE);
        #if LOOP
            fputs("langtest>", stdout);
            fgets(input, MAX_STRING_SIZE, stdin);
        #else
            #if 0
            char testStr[MAX_STRING_SIZE] =
                "int x = 1;\n"
                "int y = 2;\n"
                "int z = 0;\n"
                "z = x + y;\n"
        ...
        printTokens(tokens, tokenCount, varStorage);
