/*
    DAY 2
    https://www.youtube.com/watch?v=O-L-hF3YcVs

    fixed a bug in the parser - 1+1 skipped the + and ends up wtih 2 tokens for the 1s
    added recursive descent to construct an AST of simple math expressions
    also traverse that AST and evaluate the expressions to output results of those math expressions
    we can handle () + - / *

    This roughly touches on chapters 6 and 7 from the amazing book 
